...
train = dict()
with ReadHelper('scp:featstrain.scp') as reader:
...
test = dict()
with ReadHelper('scp:featstest.scp') as reader:
    for key, feats in reader:
        test[key] = feats
print(len(test))

dev = dict()
with ReadHelper('scp:featsdev.scp') as reader:
    for key, feats in reader:
        dev[key] = feats
print(len(dev))
...
trimodel = Sequential()
trimodel.add(Dense(1024, activation='relu', input_shape=(440,)))
trimodel.add(Dense(1024, activation='relu'))
...
def tri_generator():
    ...
    #OHE = to_categorical(triarray, nTriTargets)
    while True:
...
#history = trimodel.fit_generator(generator=(tri_generator()), steps_per_epoch=203, epochs=15, use_multiprocessing=False, validation_data=validation_generator(), validation_steps=22, shuffle=True)
#THIS IS THE ACUTAL ONE:
#history = trimodel.fit_generator(generator=(tri_generator()), steps_per_epoch=87, epochs=15, use_multiprocessing=False, validation_data=validation_generator(), validation_steps=10, shuffle=True)
#TRIAL:
history = trimodel.fit_generator(generator=(tri_generator()), steps_per_epoch=87, epochs=1, use_multiprocessing=False, validation_data=validation_generator(), validation_steps=10, shuffle=False)

'''
fig, ax = plt.subplots()
plt.plot(history.history['accuracy'], marker="o")
plt.plot(history.history['val_accuracy'], marker="*",)
plt.title('Results on TIMIT')
plt.ylabel('Accuracy')
plt.xlabel('Epoch')
ax.grid()
...
#sns.heatmap(cm_df, annot=True,annot_kws={"size": 10},fmt="d", cmap="YlGnBu")
sns.heatmap(cm_df, annot=True,annot_kws={"size": 8},fmt="d", cmap="Blues")
#sns.heatmap(cm_df.corr(), annot=True,annot_kws={"size": 8}, cmap="Blues") normalized
    ...
    if (predictedClass[i] != targetClass[i]):
        ...
        for index, nested_list in enumerate(statesValues):
            if predictedClass[i] in nested_list and targetClass[i] in nested_list:
                ...
                correctIndexStates.append(index)
                #print('predicted class: ', predictedClass[i])
                #print('actual class: ', targetClass[i])
                #print("Both integers are in statesValues[{index}]")
                #print("List is " + str(statesValues[index]))

#print(correctIndexStates) prints correct phoneme number 0-47

totalIndexStates = []
nestedlist = []
for index, nested_list in enumerate(statesValues):
    totalIndexStates.append(index)
    nestedlist.append(nested_list)
...
print('Phoneme recognition accuracy (SS): ', newAccuracy)
...
#predictedclasses = trimodel.predict(x_tri)
targetClass = trimodel.predict_classes(x_tri)
predTarget =trimodel.predict(x_tri)
print(targetClass.shape)

#target for CM:
CMtargetClass =  trimodel.predict_classes(x_test)


trimodel.save('model2.h5')
del trimodel


student2 = load_model('model2.h5')
student2.summary()
...
history2 = student2.fit_generator(generator=(student_generator()), steps_per_epoch=87, epochs=1, use_multiprocessing=False, validation_data=validation_generator(), validation_steps=10, shuffle=True)

#history2 = student2.fit(x=x_tri, y=predTarget, validation_data=(x_val, y_val),  batch_size=256, epochs=15, verbose=1, shuffle =True)
...
predictedClass = student2.predict_classes(x_tri)

#predictedClass for CM:
CMpredictedClass = student2.predict_classes(x_test)
...
for i in range(len(predictedClass)):
    if predictedClass[i] != targetClass[i]:
        ...
        if mapedState[predictedClass[i]] == mapedState[targetClass[i]]:
            stateInPhoneme.append(predictedClass[i])

###Recalculating accuracy
correct = 0
for j in range(len(predictedClass)):
    if predictedClass[j] == targetClass[j]:
        correct +=1
print('correct: ', correct)
...
#Phoneme recognition:
newAccuracy = 100 * (correctPhonemes/len(predictedClass))
print('Phoneme recognition accuracy (SSL): ', newAccuracy)

'''
fig, ax = plt.subplots()
plt.plot(history.history['accuracy'], marker="o", label='Training accuracy')
plt.plot(history.history['val_accuracy'], marker="o",label='Validation accuracy')
plt.plot(history2.history['accuracy'], marker="*", label='Training accuracy')
plt.plot(history2.history['val_accuracy'], marker="*", label='Validation accuracy')
plt.title('Results on TIMIT' )
plt.ylabel('Accuracy')
plt.xlabel('Epoch')
ax.grid()
...
plt.show()
plt.savefig('SMearlystopping3.png')
...
#################   CONFUSION MATRIX SSL   ################
SSLtarg=[]
SSLpred=[]

for i in range(len(CMtargetClass)):
    for index, nested_list in enumerate(statesValues):
        if CMtargetClass[i] in nested_list:
            SSLtarg.append(index)
        if predictedClass[i] in nested_list:
            CMSSLpred.append(index)
print(len(SSLtarg))
print(len(SSLpred))
y = SSLtarg
y_pred = SSLpred
